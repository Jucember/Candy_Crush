...
public class Explossion4X {
    public static Random random = new Random();

    static public boolean explossion4X(GameBoard gameBoard, ArrayList<TextEffect> textEffectsArr) {
        int n =0;
        for(int i = 0; i < CongfigGame.getLengthColum(); i++) {
            for(int j = 0; j < CongfigGame.getLengthRow(); j++) {
                if(gameBoard.Board[i][j].typeCandy != 10) {
                    if((j + 3) < CongfigGame.getLengthColum()) {
                        if(gameBoard.Board[i][j].typeCandy == gameBoard.Board[i][j + 1].typeCandy && gameBoard.Board[i][j].typeCandy == gameBoard.Board[i][j + 2].typeCandy &&
                                gameBoard.Board[i][j].typeCandy == gameBoard.Board[i][j + 3].typeCandy ) {
                            ...
                            AddText.addtext(500, 50 , gameBoard.Board[i][j].typeCandy, 1, "+ 1 second", 0,textEffectsArr);
